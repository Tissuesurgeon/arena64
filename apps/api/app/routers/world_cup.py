...
_SNAPSHOT = _DATA / "world_cup_2026.json"
...
async def world_cup_overview(
    ...
    """Current WC 2026 snapshot, fun facts, and shared knowledge bank entries."""
    snapshot = _load_json(_SNAPSHOT)
    history_raw = _load_json(_HISTORY)
    tournaments = list(history_raw.get("tournaments") or [])

    # Prefer live 2026 knowledge, then general world-cup, then history
    priority = case(
